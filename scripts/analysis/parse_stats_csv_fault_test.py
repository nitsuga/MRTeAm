...
field_names = [
    ...
    'ROBOT1_AWARDED_TASK',
    'ROBOT1_COMPLETED_TASK',
    'ROBOT1_UNCOMPLETED_TASK',
    'ROBOT1_SUCCESS_RATE',
    'ROBOT1_TASK_COMPLETION',
    'ROBOT2_AWARDED_TASK',
    'ROBOT2_COMPLETED_TASK',
    'ROBOT2_UNCOMPLETED_TASK',
    'ROBOT2_SUCCESS_RATE',
    'ROBOT2_TASK_COMPLETION',
    'ROBOT3_AWARDED_TASK',
    'ROBOT3_COMPLETED_TASK',
    'ROBOT3_UNCOMPLETED_TASK',
    'ROBOT3_TASK_COMPLETION',
    'ROBOT3_SUCCESS_RATE'
]
...
class Robot(object):
	...
	self.awarded_task = []
	self.completed_task = []
	self.uncompleted_task = []
	self.task_completion = True
	self.uncompleted_task_number = 0	
...
def parse_stats(bag_paths, output):
	...
	# Clean data, make sure FAULT_TYPE and SCENARIO_ID get proper strings fit in 
	...
	scenario_id = scenario_id.replace('-{0}'.format(fault_type), '')
        ...
        # 'START_CONFIG', 'MECHANSIM', 'SCENARIO_ID', 'FAULT_TYPE'
#        row_fields.extend([map, start_config, mechanism, scenario_id])
	...
	row_fields['FAULT_TYPE'] = fault_type
...
#        print('EXECUTION_PHASE_TIME:')
        ...
        exec_intervals = _get_intervals(mrta.msg.ExperimentEvent.BEGIN_EXECUTION,
                                        ...
                                        exp_msgs,
                                        'execution')
	# print('####exec_intervals = {0}'.format(exec_intervals))
	...
	total_uncompleted_task_number = 0 
        ...
        for r_name in ROBOT_NAMES:
            ...
            robot.name = r_name
            robots[r_name] = robot
	    #print("r_name is {0} and it is type {1}".format(r_name), type(r_name))
		...
		print("{0} is a {1}".format(task.task.task_id, type(int(task.task.task_id))))

                print("{0}'s task list: {1}".format(award_msg.robot_id, pp.pformat(robots[award_msg.robot_id].tasks)))
		robot.awarded_task = pp.pformat(robots[award_msg.robot_id].tasks)
...
#            print('{0} delay time:'.format(r_name))
            ...
            for status_msg in r_status_msgs:
			    ...
			    robot.completed_task.append(status_msg.task_id)
		...
		robot.uncompleted_task = pp.pformat(robot.tasks)
		# robot.uncompleted_task_number = len(robot.uncompleted_task)/5
		# print('uncompleted_task_number = {0}'.format(robot.uncompleted_task_number))
		robot.task_completion = False
                inconsistent_tasks = True
...
#                continue
            ...
            moving_intervals = _get_intervals(mrta.msg.TaskStatus.MOVING,
                                              ...
                                              r_status_msgs,
                                              'moving')
	    #print('####moving_intervals before normalize = {0}'.format(moving_intervals))
            _normalize_times(moving_intervals, exp_begin_time)

	    print('moving_intervals = {0}'.format(moving_intervals))
	    #print('exp_begin_time = {0}'.format(exp_begin_time))
	    ...
	    task_completion_intervals = _get_intervals(mrta.msg.TaskStatus.MOVING,
						      [mrta.msg.TaskStatus.SUCCESS],
                                                      r_status_msgs,
                                                      'task_completion')
	    _normalize_times(task_completion_intervals, exp_begin_time)
	    print('task_completion_intervals = {0}'.format(task_completion_intervals))

	    # 'failure_intervals' intervals
	    failure_intervals = _get_intervals(mrta.msg.TaskStatus.MOVING,
						      [mrta.msg.TaskStatus.FAILURE],
                                                      r_status_msgs,
                                                      'failure')
	    _normalize_times(failure_intervals, exp_begin_time)
	    print('failure_intervals = {0}'.format(failure_intervals))

	    # 'begin_success' intervals
	    begin_success_intervals = _get_intervals(mrta.msg.TaskStatus.BEGIN,
						      [mrta.msg.TaskStatus.SUCCESS],
                                                      r_status_msgs,
                                                      'begin_success')
	    _normalize_times(begin_success_intervals, exp_begin_time)
	    print('begin_success_intervals = {0}'.format(begin_success_intervals))
	    ...
	    total_uncompleted_task_number += robot.uncompleted_task_number
	...
	#row_fields['TOTAL_SUCCESS_RATE'] = TASK_IDS  # 'TOTAL_SUCCESS_RATE'
	    ...
	    row_fields['ROBOT{0}_AWARDED_TASK'.format(j)] = robot.awarded_task        # 'ROBOT<n>_AWARDED_TASK'
	    row_fields['ROBOT{0}_COMPLETED_TASK'.format(j)] = robot.completed_task    # 'ROBOT<n>_COMPLETED_TASK'
	    row_fields['ROBOT{0}_UNCOMPLETED_TASK'.format(j)] = robot.uncompleted_task# 'ROBOT<n>_UNCOMPLETED_TASK'
	    row_fields['ROBOT{0}_TASK_COMPLETION'.format(j)] = robot.task_completion  # 'ROBOT<n>_TASK_COMPLETION'
	    row_fields['ROBOT{0}_SUCCESS_RATE'.format(j)] = 5 * len(robot.completed_task)/len(robot.awarded_task)        # 'ROBOT<n>_AWARDED_TASK'
