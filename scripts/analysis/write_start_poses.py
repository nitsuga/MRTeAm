...
def print_start_poses(bag_paths):
    ...
    starts_out = open('robot_start_position_list.txt', 'wb')
    scenarios_out = open('scenario_id_list.txt', 'wb')
    ...
    for i, bag_path in enumerate(bag_paths):
        ...
        scenarios_lines.append(scenario_id)
        ...
        for r_name in ROBOT_NAMES:
            robot = robots[r_name]
            start_coords.append(robot.startx)
            start_coords.append(robot.starty)

        starts_lines.append(' '.join([str(f) for f in start_coords]))

        starts_line = ' '.join([str(f) for f in start_coords])

        starts_out.write('{0}\n'.format(starts_line))
        starts_out.flush()

        scenarios_out.write('{0}\n'.format(scenario_id))
        scenarios_out.flush()
