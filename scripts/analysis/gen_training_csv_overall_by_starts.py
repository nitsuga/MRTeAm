...
def write_training_files(in_file, out_dist, out_run_time, out_execution_phase_time, out_minimax, task_dir):
    ...
    try:
        ...
        out_clustered = open('overall-clustered.csv', 'wb')
        out_distributed = open('overall-distributed.csv', 'wb')
        out_overall_clustered_csv = csv.writer(out_clustered)
        out_overall_distributed_csv = csv.writer(out_distributed)

        out_overall_clustered_csv.writerow(OUT_FIELDNAMES)
        out_overall_distributed_csv.writerow(OUT_FIELDNAMES)
        ...
        row_num = 0
        for name, group in psi_ssi.groupby([stats_frame.START_CONFIG]):
            ...
            if name == 'clustered':
                out_overall_csv = out_overall_clustered_csv
                out_file = out_clustered
            elif name == 'distributed':
                out_overall_csv = out_overall_distributed_csv
                out_file = out_distributed

            for idx, row in group.iterrows():
                row_num += 1
                print "{0}, row {1}".format(name, row_num)
                # Read the task file
                task_file = row.TASK_FILE
                if task_file not in target_point_configs:
                    read_point_config(task_dir, task_file)

                row_max_dist = max(row.ROBOT1_DISTANCE, row.ROBOT2_DISTANCE, row.ROBOT3_DISTANCE)
                ...
                for pair in combinations(ROBOT_NAMES, 2):
                    ...
                    source_point = mrta.Point(row["{0}_STARTX".format(first_robot_name.upper())],
                                              row["{0}_STARTY".format(first_robot_name.upper())])
                    source_pose = planner_proxy._point_to_pose(source_point)

                    target_point = mrta.Point(row["{0}_STARTX".format(second_robot_name.upper())],
                                              row["{0}_STARTY".format(second_robot_name.upper())])
                ...
                assigned_robot_median_distances = [row.ROBOT1_DISTANCE_TO_ASSIGNED_MEDIAN,
                                                   row.ROBOT2_DISTANCE_TO_ASSIGNED_MEDIAN,
                                                   row.ROBOT3_DISTANCE_TO_ASSIGNED_MEDIAN]
                ...
                median_task_ids = row.MEDIAN_TASK_IDS.split('-')

                task_dict = target_point_configs[row.TASK_FILE]
                ...
                for median_task_id in median_task_ids:
                    ...
                    for robot_name in ROBOT_NAMES:
                        target_point = mrta.Point(row["{0}_STARTX".format(robot_name.upper())],
                                                  row["{0}_STARTY".format(robot_name.upper())])
                        target_pose = planner_proxy._point_to_pose(target_point)
                ...
                # out_overall_csv.writerow([min_dist_row[f] for f in OUT_FIELDNAMES])

                out_overall_csv.writerow([row[f] for f in OUT_FIELDNAMES])
                out_file.flush()
            ...
            # out_overall_csv.writerow([min_dist_row[f] for f in OUT_FIELDNAMES])
            # out_overall_csv.writerow([max_dist_row[f] for f in OUT_FIELDNAMES])
